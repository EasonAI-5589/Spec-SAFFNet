...
from network.mlp import MLP_3_hidden, MLP
...
elements_dict = {
    ...
    'Pb': {'model': 'MLP','lr': 0.0001, 'num_epochs': 2800, 'hidden_layers': [32,16]},
...
# 加载保存好的数据集
train_data = np.load(f'./dataset/全光谱pca/pca_data_train.npy')
train_label = np.load(f'./dataset/全光谱/full_{current_element}_label_train.npy')
test_data = np.load(f'./dataset/全光谱pca/pca_data_test.npy')
...
# 读取之前的模型评估结果
model_path = f'./model/全光谱/mlp+pca/best_model_{current_element}.pth'
results_path = f'./model/全光谱/mlp+pca/best_model_{current_element}.txt'
