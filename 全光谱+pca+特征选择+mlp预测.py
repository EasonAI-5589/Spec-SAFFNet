...
from network.mlp import MLP_3_hidden
...
elements_dict = {
    ...
    'Zn': {'lr': 0.0015, 'num_epochs': 2000, 'hidden_layers': [512, 256, 128]},
    'Pb': {'lr': 0.002, 'num_epochs': 2200, 'hidden_layers': [512, 256, 256 ]},
    'V': {'lr': 0.0012, 'num_epochs': 2000, 'hidden_layers': [512, 256, 128]},
}
...
# 加载保存好的数据集
train_data = np.load(f'./dataset/全光谱pca特征选择/pca_selected_data_train.npy')
train_label = np.load(f'./dataset/全光谱/full_{current_element}_label_train.npy')
test_data = np.load(f'./dataset/全光谱pca特征选择/pca_selected_data_test.npy')
...
# 读取之前的模型评估结果
model_path = f'./model/全光谱/mlp+pca+特征选择/best_model_{current_element}.pth'
results_path = f'./model/全光谱/mlp+pca+特征选择/best_model_{current_element}.txt'
