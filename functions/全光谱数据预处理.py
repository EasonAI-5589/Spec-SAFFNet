...
def pca_added(data,n_components=10):
    """
    contact PCA降维的特征
    """
    ...
    # 将PCA数据拼接到原始数据后面,例如从(57*2048)变成(57*2058)
    data = np.concatenate((data, pca_data), axis=1)

    return data, pca_data

def selected_added(data,k=10):
    """
    contact 特征选择的特征
    """
    k_best = SelectKBest(f_regression, k=k)
    selected_data = k_best.fit_transform(data, full_cu_label.ravel())
    
    # 将特征选择的数据拼接到原始数据后面,例如从(57*2048)变成(57*2058)
    data = np.concatenate((data, selected_data), axis=1)
...
if __name__ == '__main__':
    ...
    # 全光谱+PCA
    pca_added_data,_ = pca_added(full_data,n_components=15)

    # 全光谱+特征选择
    selected_added_data,_ = selected_added(full_data,k=15)

    # 全光谱+PCA+特征选择
    pca_selected_added_data,_ = selected_added(pca_added_data,k=10)
    ...
    # 全光谱数据标准化化
    # 多次实验过后发现,对于全光谱数据不需要标准化,标准化反而会使得训练效果很差
    ...
    pca_added_data_train, pca_added_data_test = pca_added_data[train_indices], pca_added_data[test_indices]
    selected_added_data_train, selected_added_data_test = selected_added_data[train_indices], selected_added_data[test_indices]
    pca_selected_added_data_train, pca_selected_added_data_test = pca_selected_added_data[train_indices], pca_selected_added_data[test_indices]
    ...
    np.save('./dataset/全光谱pca/pca_data_train.npy', pca_added_data_train)
    np.save('./dataset/全光谱pca/pca_data_test.npy', pca_added_data_test)

    np.save('./dataset/全光谱特征选择/selected_data_train.npy', selected_added_data_train)
    np.save('./dataset/全光谱特征选择/selected_data_test.npy', selected_added_data_test)

    np.save('./dataset/全光谱pca特征选择/pca_selected_data_train.npy', pca_selected_added_data_train)
    np.save('./dataset/全光谱pca特征选择/pca_selected_data_test.npy', pca_selected_added_data_test)
