...
# 加载保存好的数据集
train_data = np.load(f'./dataset/特征降维/full_data_train.npy')
train_label = np.load(f'./dataset/全光谱/full_{current_element}_label_train.npy')
test_data = np.load(f'./dataset/特征降维/full_data_test.npy')
...
# 读取之前的模型评估结果
model_path = f'./model/quanguangpu/feature_decomposition/best_model_{current_element}.pth'
results_path = f'./model/quanguangpu/feature_decomposition/best_model_{current_element}.txt'
